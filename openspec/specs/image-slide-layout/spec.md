...
Define the `layout.image` YAML frontmatter type that renders a slide with a single image as its primary content. Supports two display modes: `fullscreen` (image fills the entire slide in PPT mode, constrained to content area in DOC mode) and `content` (image constrained to the 16:9 content area). Works in both DOC and PPT viewing modes.
