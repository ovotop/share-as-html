...
The CSS template SHALL include rules for `.slide-image-fullscreen` and `.slide-image-content` classes to support the `layout.image` YAML type. The JS template requires no modification — image slides behave as normal slides within the existing state machine (no focus, no zoom).
