...
Define the Markdown-based file format for authoring share-as-html presentation projects. Presentations are organized as directories with `meta.md` for metadata, `outline.md` for section planning, and `slides/*.md` files for individual slides. Layout is controlled via YAML frontmatter, keeping Markdown body content clean and human-readable.
