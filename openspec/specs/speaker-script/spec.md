...
Define the standalone speaker script file format. The speaker script is a Markdown file that records the presenter's spoken words and stage directions. It is separate from the presentation content and is never rendered in the output HTML — it is for the presenter's eyes only.
