...
Define how a single Markdown source produces two distinct viewing experiences: PPT mode (visual-only slides for projection) and DOC mode (visual blocks embedded in reader-facing prose for article reading). The same source file separates visual content from reader narrative using HTML comment markers.
