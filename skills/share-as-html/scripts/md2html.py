...
body.mode-zoom .zoom-container.bounce-down { animation: slide-bounce-down 0.3s ease-out; }
body.mode-zoom .zoom-container.bounce-up { animation: slide-bounce-up 0.3s ease-out; }
body.mode-zoom .zoom-container.bounce-left { animation: slide-bounce-left 0.3s ease-out; }
body.mode-zoom .zoom-container.bounce-right { animation: slide-bounce-right 0.3s ease-out; }
...
let zoomTarget = null;
let zoomScale = 1.0;
let scaleLine = [1.0, 1.25, 1.5, 2.0, 3.0];
let fillStepScale = 2.0;
    ...
    switch(currentState) {
        ...
        case 'PPT_ZOOM':
            if (e.key === 'Escape') {
                ...
                e.preventDefault();
                const idx = scaleLine.indexOf(zoomScale);
                if (idx >= 0 && idx < scaleLine.length - 1) {
                    zoomScale = scaleLine[idx + 1];
                    applyZoomScale();
                }
            } else if (e.key === '-') {
                e.preventDefault();
                ...
                e.preventDefault();
                if (!scrollZoomContainer('left')) {
                    triggerZoomBounce('left');
                }
            } else if (e.key === 'ArrowRight') {
                e.preventDefault();
                if (!scrollZoomContainer('right')) {
                    triggerZoomBounce('right');
                }
            } else if (e.key === 'ArrowUp') {
                e.preventDefault();
                if (!scrollZoomContainer('up')) {
                    triggerZoomBounce('up');
                }
            } else if (e.key === 'ArrowDown') {
                e.preventDefault();
                if (!scrollZoomContainer('down')) {
                    triggerZoomBounce('down');
                }
            }
            break;
    }
...
function enterZoomMode(element) {
    ...
    // Wait for layout then compute dynamic scale line
    requestAnimationFrame(function() {
        var rect = clone.getBoundingClientRect();
        var natW = rect.width;
        var natH = rect.height;

        // Fallback: if clone has no layout dimensions, use original element
        if (natW === 0 || natH === 0) {
            var origRect = element.getBoundingClientRect();
            natW = origRect.width || element.naturalWidth || 100;
            natH = origRect.height || element.naturalHeight || 100;
        }

        var vw = window.innerWidth;
        var vh = window.innerHeight;
        var fillX = Math.max(vw / natW, vh / natH);
        fillX = Math.max(1.25, Math.min(3.0, fillX));
        fillStepScale = fillX;

        var base = [1.0, 1.25, 1.5, 2.0, 3.0];
        var sorted = base.concat([fillX]).sort(function(a, b) { return a - b; });
        scaleLine = sorted.filter(function(x, i, arr) {
            return i === 0 || Math.abs(x - arr[i - 1]) > 0.01;
        });
...
function scrollZoomContainer(direction) {
    ...
    switch(direction) {
        ...
        case 'left':
            ...
            return true;
        case 'right':
            if (container.scrollLeft + container.clientWidth >= container.scrollWidth) return false;
            container.scrollLeft += scrollAmount;
            return true;
    }
    return false;
...
function triggerZoomBounce(direction) {
    const container = document.querySelector('.zoom-container');
    if (!container) return;
